"""Download GAN Getting Started files with the Kaggle CLI."""
...
COMPETITION_SLUG = "gan-getting-started"
COMPETITION_ROOT = Path(__file__).resolve().parents[1]
DEFAULT_RAW_DIR = COMPETITION_ROOT / "data" / "raw"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Download GAN Getting Started data.")
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="Optional comma-separated file list for partial download.",
    )
    parser.add_argument(
        "--all-files",
        action="store_true",
        help="Download all competition files (recommended for this competition).",
    )
    parser.add_argument(
        "--force",
        action="store_true",
        help="Pass --force to Kaggle CLI.",
    )
...
def ensure_kaggle_cli_available() -> str:
    ...
    if kaggle_exe is None:
        raise RuntimeError(
            "Kaggle CLI was not found on PATH. Activate conda env or install the `kaggle` package first."
        )
    return kaggle_exe
...
def build_download_error_message(competition: str, output: str) -> str:
    if "401" in output and "Unauthorized" in output:
        return (
            "Kaggle CLI returned 401 Unauthorized.\n"
            "Check ~/.kaggle credentials and try `kaggle competitions list -s titanic`.\n"
            f"Then open https://www.kaggle.com/competitions/{competition}.\n\n"
            f"Original output:\n{output}"
        )
    if "403" in output and "Forbidden" in output:
        return (
            "Kaggle CLI returned 403 Forbidden.\n"
            "Join/accept rules for this competition in browser and retry.\n\n"
            f"Original output:\n{output}"
        )
...
def main() -> None:
    ...
    all_files = bool(args.all_files or not requested_files)
    ...
    root_items = sorted(path for path in args.raw_dir.iterdir())

    print(f"Downloaded competition files into: {args.raw_dir}")
    print(f"Download mode: {'all files' if all_files else 'selected files'}")
    if extracted:
        print(f"Extracted {len(extracted)} archive members.")
