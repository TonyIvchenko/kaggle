...
from competitions.titanic.scripts.download_data import (
...
def test_parse_file_list_deduplicates_and_ignores_whitespace():
    files = parse_file_list("train.csv, test.csv, train.csv, ,gender_submission.csv")
    assert files == ["train.csv", "test.csv", "gender_submission.csv"]
...
def test_build_download_commands_uses_selected_files(tmp_path: Path):
    ...
    assert commands == [
        ["/usr/bin/kaggle", "competitions", "download", "-c", "titanic", "-p", str(tmp_path), "-f", "train.csv"],
        ["/usr/bin/kaggle", "competitions", "download", "-c", "titanic", "-p", str(tmp_path), "-f", "test.csv"],
    ]
...
def test_build_download_commands_all_files_mode(tmp_path: Path):
    ...
    assert commands == [
        ["/usr/bin/kaggle", "competitions", "download", "-c", "titanic", "-p", str(tmp_path), "--force"]
    ]
...
def test_build_download_error_message_401_mentions_credentials():
    message = build_download_error_message("titanic", "401 Client Error: Unauthorized")
...
def test_build_download_error_message_403_mentions_identity_verification():
    message = build_download_error_message("titanic", "403 Client Error: Forbidden")
