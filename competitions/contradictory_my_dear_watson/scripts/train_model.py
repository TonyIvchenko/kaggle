"""Train and evaluate Contradictory, My Dear Watson baseline model."""
...
from competitions.contradictory_my_dear_watson.models.baseline import (  # noqa: E402
    COMPETITION_SLUG,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train Contradictory, My Dear Watson baseline model.")
...
def main() -> None:
    ...
    dataset = build_dataset(files, target_column=args.target_column)

    args.processed_dir.mkdir(parents=True, exist_ok=True)
    holdout_predictions_path = args.processed_dir / "holdout_predictions.csv"
    preview_path = args.processed_dir / "train_preview.csv"
    dataset["train_frame"].head(1000).to_csv(preview_path, index=False)
    ...
    final_model = fit_final_model(dataset=dataset, seed=args.seed)
    ...
    print(f"Holdout accuracy: {holdout_metrics['accuracy']:.6f}")
