"""Download Store Sales files with the Kaggle CLI."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Download Store Sales competition data.")
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="Optional comma-separated file list.",
    )
    parser.add_argument(
        "--all-files",
        action="store_true",
        help="Download all competition files instead of the default working set.",
    )
    parser.add_argument("--force", action="store_true", help="Pass --force to the Kaggle CLI download command.")
...
def build_download_error_message(competition: str, output: str) -> str:
    if "401" in output and "Unauthorized" in output:
        return (
            ...
            "Credentials are invalid or expired, or competition access is not accepted.\n\n"
            "Debug in order:\n"
            "  - Run: kaggle competitions list -s store-sales\n"
...
def main() -> None:
    ...
    print("Download mode: " + ("all files" if args.all_files else f"selected files ({', '.join(requested_files)})"))
    if extracted:
