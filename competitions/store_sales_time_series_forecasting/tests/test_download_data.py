...
from competitions.store_sales_time_series_forecasting.scripts.download_data import (
...
def test_parse_file_list_deduplicates_and_ignores_whitespace():
    files = parse_file_list("train.csv, test.csv, train.csv, ,stores.csv")
    assert files == ["train.csv", "test.csv", "stores.csv"]
...
def test_build_download_error_message_401_mentions_credentials():
    message = build_download_error_message("store-sales-time-series-forecasting", "401 Client Error: Unauthorized")
    assert "Credentials are invalid or expired" in message
    assert "kaggle competitions list -s store-sales" in message


def test_build_download_error_message_403_mentions_identity_verification():
    message = build_download_error_message("store-sales-time-series-forecasting", "403 Client Error: Forbidden")
